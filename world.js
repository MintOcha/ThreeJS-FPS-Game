...
window.game.createWorld = function () {
  ...
  // Load carpet texture
  const carpetTexture = new BABYLON.Texture("assets/carpet.png", g.scene);
  // Scale texture for proper tiling across the large ground plane
  carpetTexture.uScale = groundSize / 8; // Repeat every 8 units for good detail
  carpetTexture.vScale = groundSize / 8;
  carpetTexture.wrapU = BABYLON.Texture.WRAP_ADDRESSMODE;
  carpetTexture.wrapV = BABYLON.Texture.WRAP_ADDRESSMODE;

  // Apply carpet texture with realistic properties
  groundMaterial.diffuseTexture = carpetTexture;
  groundMaterial.diffuseColor = new BABYLON.Color3(0.9, 0.9, 0.9); // Neutral tint to preserve texture colors
  groundMaterial.specularColor = new BABYLON.Color3(0.05, 0.05, 0.05); // Low specular for carpet
  groundMaterial.roughness = 0.9; // High roughness for carpet material
  ...
  g.CollisionManager.createWorldCollision(ground, groundAggregate, {
    type: "ground",
    name: "ground",
  });
...
window.game.createWall = function (x, y, z, width, height, depth, material) {
  ...
  g.CollisionManager.createWorldCollision(wall, wallAggregate, {
    type: "wall",
    name: wall.name,
  });
